...
namespace ID3
{
    public class Tree
    {
        ...
        private static TreeNode GetRootNode(DataTable data, string edge) //获取根节点
        {
            var attributes = new List<MyAttribute>();
            var highestInformationGainIndex = -1;
            var highestInformationGain = double.MinValue;

            // Get all names, amount of attributes and attributes for every column             //获取 所有的名字；属性数量；每行的属性
            
            for (var i = 0; i < data.Columns.Count - 1; i++)
            {
                var differentAttributenames = MyAttribute.GetDifferentAttributeNamesOfColumn(data, i);
                //MessageBox.Show("differentAttributenames:" + data.Columns[i].ToString());
                attributes.Add(new MyAttribute(data.Columns[i].ToString(), differentAttributenames));
            }

            // Calculate Entropy (S)
            var tableEntropy = CalculateTableEntropy(data);// 计算 熵 的值
            MessageBox.Show("tableEntropy 熵 :" + tableEntropy);
